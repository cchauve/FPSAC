...
class Row:
	...
	def __init__(self, s, ident, weight, sp, isT, gaps):
		...
		self._gaps = gaps		# Dictionary with species as keys, mapping to a list of strings; 
								# each string gives the gaps between consecutive markers, assuming their order is fixed
								# (repeat spanning intervals,adjacencies).
	#enddef
	...
	def __str__(self):
		...
		if self._gaps!={}:
			gap_list=''
			for sp in self._gaps:	
				for occurrence in self._gaps[sp]:
					gap_list = gap_list+' #'+' '.join(occurrence)
			s = s + gap_list
		#endif
...
#					del tok[0]
				...
				while i < len(tok):
					try:
						...
						if (tok[i] == 'T'):
							...
							species=tok[i][1:tok[i].find('.')]
							if species not in gaps:
								gaps[species]=[(tok[i][1:],tok[i+1])]
							else:
								gaps[species].append((tok[i][1:],tok[i+1]))
							del tok[i]
							del tok[i]
							i-=2
						#endif
				...
				#endwhile
		        if X == -1:
				    self.add_row(set(tok), ident, weight, sp, isT, gaps)					
		        else:
	...
	# isT: True if the row is a telomere - bool
	def add_row(self, row, ident = '-1', weight = Decimal('0'), sp = [], isT = False,gaps=None):
		if ident == '-1':
			ident = str(self._height)
		#endif
	
		self._rows.append(Row(row, ident, weight, sp, isT,gaps))
